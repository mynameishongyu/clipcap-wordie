...
function buildDirectVisionSlotFillPromptPayload(input: {
  ...
  return {
    ...
    strict_requirements: [
      ...
      'For direct system-time slots, matches[0].evidence_text must not contain "minus one day", "subtract", "-1 day", "previous day", "减一天", or any inferred offset. It must state that the value is copied directly from the Windows taskbar date.',
...
function buildReferencePageAlignmentPromptPayload(input: {
  ...
  return {
    ...
    strict_requirements: [
      ...
      'The uploaded PDF is expected to contain pages produced from the same template families as the annotated reference pages, but variable content is expected to differ.',
      'Ignore variable content when aligning pages: names, ID numbers, phone numbers, dates, amounts, signatures, seals, printed page numbers, serial numbers, customer names, product values, and handwritten values. These are not reliable matching signals.',
      'Match pages by stable visual template structure only: document/form title, fixed section headings, table/grid row-column structure, fixed field labels, page orientation, rotation, and the relative position of annotated boxes within the same fixed section.',
      ...
      'A system screenshot must only match another system screenshot with the same application layout, such as browser/app header, left navigation, account information grid, date/account sections, and bottom taskbar/status area. Never match a system screenshot to a scanned paper form, contract, certificate, ID card, notice, or registration document.',
      'A scanned paper document must only match the same document/form type. Do not match a contract page to a notice, approval form, ID card, vehicle registration certificate, or system screenshot.',
      ...
      'If values look similar but the fixed document type or layout is different, treat it as a poor match. If values differ but the fixed layout is the same, treat it as a strong match.',
      ...
      'If document type signals conflict, prefer the page with the closest title, fixed section labels, and structural layout over pages with similar individual values.',
      ...
      'reason must cite stable layout evidence, not copied variable values. It must be a short English phrase with at most 20 words that names the matching document type or fixed layout signal.',
      'Do not quote OCR text repeatedly. Do not list candidate snippets. Do not repeat names, ID-card words, labels, values, or the same phrase multiple times.',
    ],
    output_schema: {
      alignments: [
        {
